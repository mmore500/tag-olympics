...
#SBATCH --job-name mid-target-big
...
OUTPUT_DIR="/mnt/scratch/mmore500/tag-olympics-midtarget-big/run=${SLURM_ARRAY_TASK_ID}"
...
for REP in 0; do
  ...
  python3 /mnt/home/mmore500/tag-olympics/script/MidGenerateBiGraph.py         \
    -s $(( $SEED + $REP )) --lefts 32 --rights 32                              \
    --regular $(( $TARGET_REGULAR_DEGREE * 32 ))                               \
    --irregular $(( $TARGET_IRREGULAR_DEGREE * 32 ))                           \
  ...
  ./mid-tag-olympics MBM                                                       \
    ...
    -MFM_GENS 515 -MO_LENGTH 64 -MO_MUT_EXPECTED_REDRAWS $MUTATION_RATE        \
