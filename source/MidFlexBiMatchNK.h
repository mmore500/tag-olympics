...
void MidFlexBiMatchNK(const Metrics::metric_t &metric,  const Config &cfg) {
  ...
  emp::NKLandscape nk_landscape;

  std::unordered_map<std::string, size_t> uids;
  std::unordered_map<std::string, size_t> left_uids;
  std::unordered_map<std::string, size_t> right_uids;
  ...
  auto incr_instance = [&](){
    ...
    while (f.size()) {
      ...
      if (emp::keyname::unpack(res[0])["side"] == "left") {
        if (!left_uids.count(res[0])) left_uids[res[0]] = left_uids.size();
        lefts.insert(res[0]);
      } else {
        if (!right_uids.count(res[0])) right_uids[res[0]] = right_uids.size();
        rights.insert(res[0]);
      }
    ...
    // reduce variance between runs by using same nk fitness landscape
    emp::Random rand_(1);
    nk_landscape = emp::NKLandscape(
      left_uids.size() * right_uids.size(),
      cfg.MFM_NK_K(),
      rand_
    );
  ...
  grid_world.SetupFitnessFile(emp::keyname::pack({
    ...
    {"target-k", emp::to_string(cfg.MFM_NK_K())},
  ...
  if (cfg.MFM_RANKED()) {
    grid_world.SetFitFun(
      ...
      (MidOrganism<Config::BS_WIDTH()> & org){
        ...
        emp::BitVector match_matrix(left_uids.size() * right_uids.size());
        ...
        for (const auto & left : lefts) {
          ...
          for (const auto & right : resp) {
            match_matrix[
              left_uids[left]
              + right_uids[right] * left_uids.size()
            ] = 1;
          }

        }

        return nk_landscape.GetFitness(match_matrix);
  ...
  auto df_neut = emp::DataFile(emp::keyname::pack({
    ...
    {"target-degree", cfg.MFM_TARGET_DEGREE()},
    {"target-size", emp::to_string(lefts.size() + rights.size())},
    {"target-k", emp::to_string(cfg.MFM_NK_K())},   {"mut", emp::to_string(cfg.MO_MUT_EXPECTED_REDRAWS())},
    {"treatment", cfg.TREATMENT()},
    {"seed", emp::to_string(cfg.SEED())},
    {"fit-fun", cfg.MFM_RANKED() ? "ranked" : "scored"},
    // {"_emp_hash=", STRINGIFY(EMPIRICAL_HASH_)},
...
void MidFlexBiMatchNK(const Metrics::collection_t &metrics, const Config &cfg) {
  for (const auto & mptr : metrics) {
    const auto & metric = *mptr;
    std::cout << "Metric " << metric.name() << std::endl;
    MidFlexBiMatchNK(metric, cfg);
    std::cout << std::endl;
  }
}
