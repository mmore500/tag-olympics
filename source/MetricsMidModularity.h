...
struct AddDimMod {
  ...
  using pack_t = typename emp::TypePack<
    ...
    emp::HardStartMod<emp::AsymmetricWrapMetric<Config::BS_WIDTH()/Dim>>,
...
struct MetricsMidModularity {

  using collection_t = Metrics::collection_t;

  collection_t mets;

  MetricsMidModularity() {
    AddDimMod<1>::append<AddDimMod<1>::pack_t>(mets);
